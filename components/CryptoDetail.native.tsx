...
import { VictoryChart, VictoryArea, VictoryAxis, VictoryTheme, VictoryTooltip, VictoryContainer } from 'victory-native';
...
const CryptoDetailNative: React.FC<CryptoDetailProps> = ({ cryptoId, onBack }) => {
  ...
  const chartData = history.map((d) => ({ x: d.date, y: d.price }));
    ...
    <View style={styles.container}>
      ...
      <View style={styles.chartContainer}>
        <Svg width="100%" height={320}>
          ...
          >
            <VictoryAxis style={{ axis: { stroke: isDarkMode ? '#9CA3AF' : '#6B7280' } }} tickFormat={() => ''} />
            <VictoryAxis dependentAxis style={{ axis: { stroke: isDarkMode ? '#9CA3AF' : '#6B7280' } }} tickFormat={(value) => `$${(value/1000).toFixed(0)}k`} />
            <VictoryArea data={chartData} interpolation="natural" style={{ data: { fill: 'url(#gradientPrice)', stroke: '#3b82f6', fillOpacity: 1 } }} labels={({ datum }) => `$${datum.y}`} labelComponent={<VictoryTooltip />} />
          </VictoryChart>
        </Svg>
      </View>
...
const getCryptoDetailStyles = (isDarkMode: boolean) => StyleSheet.create({
    ...
    errorText: { color: isDarkMode ? '#F87171' : '#EF4444', marginTop: 8 },
    container: { flex: 1, padding: 16, backgroundColor: isDarkMode ? '#111827' : '#F9FAFB' },
    backButton: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 },
    backButtonText: { color: isDarkMode ? '#60A5FA' : '#3B82F6', marginLeft: 8 },
    header: { flexDirection: 'row', alignItems: 'center', marginBottom: 24 },
    image: { width: 48, height: 48, borderRadius: 24, marginRight: 16 },
    headerTitle: { fontSize: 24, fontWeight: 'bold', color: isDarkMode ? '#F9FAFB' : '#111827' },
    headerSubtitle: { color: isDarkMode ? '#9CA3AF' : '#6B7280', textTransform: 'uppercase' },
    priceContainer: { marginBottom: 24 },
    price: { fontSize: 36, fontWeight: 'bold', color: isDarkMode ? '#F9FAFB' : '#111827' },
    priceChange: { fontSize: 20, fontWeight: '600' },
    chartContainer: { height: 320, width: '100%', marginBottom: 32 },
    statsTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 16, color: isDarkMode ? '#F9FAFB' : '#111827' },
...
const getStatCardStyles = (isDarkMode: boolean) => StyleSheet.create({
    ...
    label: { fontSize: 14, color: isDarkMode ? '#9CA3AF' : '#6B7280' },
    value: { fontSize: 18, fontWeight: '600', color: isDarkMode ? '#F9FAFB' : '#111827' },
});

export default CryptoDetailNative;
