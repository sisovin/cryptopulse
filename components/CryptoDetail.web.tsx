...
const CryptoDetail: React.FC<CryptoDetailProps> = ({ cryptoId, onBack }) => {
  ...
  const isDarkMode = false; // web default until theme support provided
      ...
      <View style={styles.chartContainer}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={history} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            ...
            <Tooltip contentStyle={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB' }} labelStyle={{ color: '#1F2937' }} formatter={(value: number) => [`$${value.toLocaleString()}`, 'Price']} />
...
const getCryptoDetailStyles = (isDarkMode: boolean) => StyleSheet.create({
    ...
    backButtonText: { color: '#3B82F6', marginLeft: 8 },
    header: { flexDirection: 'row', alignItems: 'center', marginBottom: 24 },
    image: { width: 48, height: 48, borderRadius: 24, marginRight: 16 },
    headerTitle: { fontSize: 24, fontWeight: 'bold', color: '#111827' },
    headerSubtitle: { color: '#6B7280', textTransform: 'uppercase' },
    ...
    statsTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 16, color: '#111827' },
    statsGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' },
});

const getStatCardStyles = (isDarkMode: boolean) => StyleSheet.create({
    container: { backgroundColor: '#FFFFFF', padding: 16, borderRadius: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.22, shadowRadius: 2.22, elevation: 3, width: '48%', marginBottom: 16 },
